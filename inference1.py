# inference.py (modified for PyTorch 2.1+ and modern libraries)
import numpy as np
...
def main():
    ...
    landmark_path = f'temp/{base_name}_landmarks.txt'
    if not os.path.isfile(landmark_path) or args.re_preprocess:
        print('[Step 1] Landmarks Extraction in Video.')
        kp_extractor = KeypointExtractor()
        lm = kp_extractor.extract_keypoint(frames_pil, landmark_path)
    else:
        print('[Step 1] Using saved landmarks.')
        lm = np.loadtxt(landmark_path).astype(np.float32)
        lm = lm.reshape([len(full_frames), -1, 2])

    coeffs_path = f'temp/{base_name}_coeffs.npy'
    if not os.path.isfile(coeffs_path) or args.exp_img is not None or args.re_preprocess:
    ...
    # Skipping expression_img and synthesis for brevity
    print('[Info] Inference preparation complete.')
    print('[Note] You can now proceed to run the model as per your setup.')
